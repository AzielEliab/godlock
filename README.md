...
- Suite mesh (QNM-BUILD-1.0, default off, live|locked|isolated counts only, not an anonymity network). SPLIT THE WIRES. COLD-COPY SURVIVAL. Phoenix local only — die-with-pull does not bring .uk back. QNS-CD-1.0 photon QNS1 packet transfer (hub cite / Worker mesh cross-map only; local qnsd in https://github.com/AzielEliab/qnm-node; runtime cites in https://github.com/AzielEliab/aziel-runtime; no public qnsd proxy): https://godlock.uk/runtime/v1/mesh
