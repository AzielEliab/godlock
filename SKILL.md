...
- GodLock Softwares (GodLock-first heading → list; first card GodLock, Aziel Runtime secondary / Try on Glama; official listing https://www.azieleliab.com/software; not Digital Library completeness; launch readiness after the list — SoT main 6a3798a / version_id 105fa1ee / 2.0.0-rc1; human UI + MCP fraggate_call + FragGate sole door + mesh/AZVPN HTTPS/WS REAL, WG/OpenVPN SLOT + radios worker_hardware:false + /download suite pack; never ; GodLock is a challenge stress-test, not a VPN identity): https://godlock.uk/software
...
- Runtime **2.0.0-rc1** door (SoT LIVE main 6a3798a / version_id 105fa1ee): https://godlock.uk/runtime
- Try on Glama: https://glama.ai/mcp/servers/AzielEliab/aziel-runtime
...
- Live software catalog (Worker SSoT): `GET https://aziel-runtime.vibelock.workers.dev/v1/software` (same-origin `GET https://godlock.uk/runtime/v1/software`). FragGate list is fallback only (`GET /v1/fraggate/list`). GodLock `GET https://godlock.uk/v1/software` is hub-local (Aziel Runtime + sister_cites), not the suite catalog. Whitestone lives on Worker SSoT — not a FragGate door.
