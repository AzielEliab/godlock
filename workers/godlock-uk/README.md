...
- `GET /v1/mesh` and `GET /v1/mesh/status` same-origin proxies (parity with azieleliab.com / corpus hubs) so Live Nodes clients that hit `/v1/mesh/status` work. GET never enables. Writes stay on `/runtime/v1/mesh/{enable|join|leave}`. Remain-OFF.
