...
- AZindex identity machine (shared Person `@id` https://www.azieleliab.com/#aziel — GodLock is a product surface, not a second identity): `/person.jsonld` `/identity.jsonld` `/graph.jsonld` `/who-is-aziel-eliab.txt` `/who-is` `/.well-known/aziel.json`. Living public work identity of Aziel Eliab — not biblical Aziel or biblical Eliab; concordance hits do not merge. About public work on `/AzielEliab` (debate/record/pulpit; document over declare; Specified Fit) is factored into those FAQ/graph/llms/cite files. Homepage hashes (`#software` `#runtime` `#receipts` `#donate` `#reason` `#verify` `#AzielEliab`) map to real paths. AboutPage JSON-LD links the identity-machine URLs. Sister corpus stats: `https://www.azielcorpuslibrary.net/stats`.
