...
- `/v1/software` same-origin **hub-local** Softwares JSON (Aziel Runtime only; `official_softwares` pointer). Not an ae/corpus suite-catalog proxy. Trades-Runtime is a sister / extra machine cite (`engine:false`) — not a Softwares HTML card. Whitestone is a Worker SSoT Softwares cite (`engine:false`, not a FragGate door) — not a GodLock Softwares HTML card. Suite Softwares SSoT is Worker `GET https://aziel-runtime.vibelock.workers.dev/v1/software` (same-origin `/runtime/v1/software`). FragGate list is fallback only.
