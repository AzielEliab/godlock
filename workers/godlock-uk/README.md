...
- `/AzielEliab` public identity page (Aziel Eliab only); visible 1 Chronicles 15:20 lock under the author line; `/aziel-eliab`, `/about`, `/aboutme` 308 here
- `/who` visible HTML identity lock (200, H1 `Who is Aziel Eliab`); `/who-is` 308 to `/who-is-aziel-eliab.txt`
...
- AZindex identity machine (shared Person `@id` https://www.azieleliab.com/#aziel — GodLock is a product surface, not a second identity): `/who` `/person.jsonld` `/.well-known/person.jsonld` `/identity.jsonld` `/graph.jsonld` `/who-is-aziel-eliab.txt` `/who-is` `/.well-known/aziel.json`. Living researcher and software designer named Aziel Eliab (one person). Not the two Levitical musicians Aziel and Eliab named together in 1 Chronicles 15:20. Not euaziel.site; not Aziel S. (Flutter/portfolio); not other engineers named Aziel. Visible lock line is in HTML body on `/who` and under the `/AzielEliab` author line. Homepage hashes (`#software` `#runtime` `#receipts` `#donate` `#reason` `#verify` `#AzielEliab`) map to real paths. AboutPage JSON-LD links the identity-machine URLs. Sister corpus stats: `https://www.azielcorpuslibrary.net/stats`.
