...
describe("Aziel Eliab page chrome", () => {
  ...
  it("orders nav Engine | Reason | Softwares | Runtime | Receipts | Verify | Donate | Aziel Eliab | Aziel Corpus Library | He Didn't Jump", () => {
    ...
    assert.match(
      nav,
      /href="\/verify"[^>]*>Verify<\/a><span class="sep">\|<\/span><a href="\/donate">Donate<\/a><span class="sep">\|<\/span><a href="\/AzielEliab" class="aziel">Aziel Eliab<\/a><span class="sep">\|<\/span><a href="https:\/\/www\.azielcorpuslibrary\.net\/AzielEliab" class="aziel">Aziel Corpus Library<\/a><span class="sep">\|<\/span><a href="https:\/\/www\.hedidntjump\.com\/" class="aziel">He Didn't Jump<\/a>/,
    );
  ...
  it("renders crawlable manifesto HTML with a Digital Library cross-link", () => {
    ...
    assert.ok((about.significantLink || []).includes("https://www.hedidntjump.com/"));
...
describe("Aziel Eliab SEO surfaces", () => {
  ...
  it("lists identity and library pages in robots, sitemap, cite, llms, and ai", async () => {
    ...
    assert.match(robots, /MCP discovery: \/\.well-known\/mcp\.json \/mcp\.json → POST \/runtime\/mcp\./);
    ...
    assert.ok(xml.includes(CANON_HOST + "/software#godlock"));
    assert.ok(xml.includes(CANON_HOST + "/software#aziel-runtime"));
    assert.ok(xml.includes(OFFICIAL_SOFTWARES));
    ...
    assert.ok(xml.includes(CANON_HOST + "/runtime/v1/fraggate/describe?slug=peacelock"));
    assert.ok(xml.includes(CANON_HOST + "/runtime/v1/fraggate/describe?slug=azbrowser"));
    assert.ok(xml.includes(CANON_HOST + "/runtime/v1/fraggate/describe?slug=azcoherence"));
    ...
    assert.equal(cite.he_didnt_jump, "https://www.hedidntjump.com/");
    ...
    assert.match(cite.runtime_glama_note, /Verified Glama listing for AzielEliab\/aziel-runtime/);
    ...
    assert.match(cite.mcp_discovery_note, /Door remains \/runtime/);
    ...
    assert.equal(cite.mesh_rollup, "rollup.live is Softwares (software_nodes / active_nodes), not Live Nodes; Live Nodes are live_nodes / rollup.mesh");
    ...
    assert.equal(cite.software_nodes, "Softwares {slug}-worker roster (never Nodes or Live Nodes); same plane as rollup.live and active_nodes");
    ...
    assert.match(cite.anon_broadcast_note, /Local communique style tool/);
    ...
    assert.equal(cite.azcoherence, OFFICIAL_SOFTWARES + "#azcoherence");
    ...
    assert.match(cite.azcoherence_note, /Peer AZ-CLCE/);
    ...
    assert.match(llms, /MCP discovery: https:\/\/godlock\.uk\/\.well-known\/mcp\.json and https:\/\/godlock\.uk\/mcp\.json → POST https:\/\/godlock\.uk\/runtime\/mcp/);
    ...
    assert.match(llms, /Live Nodes fleet: one total from GET \/v1\/mesh live_nodes \(or rollup\.mesh; plane human-mesh-users-site-viewers\)/);
    assert.match(llms, /rollup\.live is Softwares \(software_nodes \/ active_nodes\), not Live Nodes/);
    ...
    assert.match(llms, /anon-broadcast local communique style tool/);
    ...
    assert.match(expectedMcp.note, /Discovery JSON pointing at the same-origin Runtime MCP door/);
...
describe("priority page SEO", () => {
  ...
  it("renders Softwares CollectionPage JSON-LD without changing chrome", async () => {
    ...
    assert.ok((collection.relatedLink || []).includes("https://www.hedidntjump.com/"));
    assert.ok(ld["@graph"].some((n) => n["@type"] === "BreadcrumbList"));
    const list = ld["@graph"].find((n) => n["@type"] === "ItemList");
    assert.ok(list);
    assert.equal(list.numberOfItems, 2);
    assert.equal(list.itemListElement[0].item.identifier, "godlock");
    assert.ok((list.itemListElement || []).some((it) => it.url === "https://godlock.uk/software#godlock"));
    assert.ok((list.itemListElement || []).some((it) => it.url === "https://godlock.uk/software#aziel-runtime"));
    assert.ok(!(list.itemListElement || []).some((it) => it.url === "https://godlock.uk/software#fraggate"));
    assert.ok(!(list.itemListElement || []).some((it) => it.url === "https://godlock.uk/software#trades-runtime"));
    assert.ok((list.itemListElement || []).some((it) => it.item && it.item["@type"] === "SoftwareApplication" && it.item.identifier === "aziel-runtime"));
    ...
    assert.match(html, /<article class="soft-card" id="aziel-runtime"/);
    assert.doesNotMatch(html, /<article class="soft-card[^"]*" id="fraggate"/);
    assert.doesNotMatch(html, /<title>[^<]*Trades-Runtime/);
    assert.doesNotMatch(html, /og:title" content="[^"]*Trades-Runtime/);
    assert.match(html, /https:\/\/www\.azieleliab\.com\/software/);
    assert.match(html, /name="keywords" content="GodLock Softwares, GodLock, GodLock.uk/);
...
describe("homepage stays a natural argument surface", () => {
  it("puts Specified Fit on the spine and does not coach a paste-block", async () => {
    ...
    assert.doesNotMatch(html, /Suite doors available from GodLock/);
    assert.match(html, /href="\/software#aziel-runtime">Aziel Runtime<\/a>/);
    assert.doesNotMatch(html, /href="\/software#fraggate">FragGate<\/a>/);
    assert.match(html, /https:\/\/www\.azieleliab\.com\/software/);
    assert.doesNotMatch(html, /href="\/software#aziel-corpus">Aziel Digital Library<\/a>/);
    assert.doesNotMatch(html, /<h2>Software<\/h2>/);
    assert.match(html, /href="https:\/\/www\.azieleliab\.com\/software">Official Softwares<\/a>/);
    ...
    assert.match(html, /Suite mesh: on/);
    assert.doesNotMatch(html, /class="banner"/);
    assert.doesNotMatch(html, /id="ingest-as-receipt"/);
    const visibleHomeChrome = html.replace(/<script[\s\S]*?<\/script>/gi, "");
    assert.doesNotMatch(visibleHomeChrome, /SPLIT THE WIRES|COLD-COPY SURVIVAL|REHEAL refuse|die-with-pull does not bring godlock\.uk back/);
...
describe("major AI client list", () => {
  it("names the full public client set, not the Grok/ChatGPT/Venice triad alone", () => {
    ...
    assert.doesNotMatch(homeLine, /href="\/software#fraggate">FragGate<\/a>/);
    assert.match(homeLine, /https:\/\/www\.azieleliab\.com\/software/);
    assert.doesNotMatch(homeLine, /Suite doors available from GodLock/);
    ...
    assert.ok(softwareMeta.startsWith("GodLock Softwares is GodLock-first"));
    assert.match(softwareMeta, /first card is GodLock/);
    assert.match(softwareMeta, /2\.0\.0-rc1/);
    assert.match(softwareMeta, /Claude \(Anthropic\)/);
    assert.doesNotMatch(softwareMeta, /Aziel Runtime only on this page/);
    assert.doesNotMatch(softwareMeta, /Trades-Runtime/);
    assert.ok(defaultDescription("runtime").startsWith("Aziel Runtime is a node-meshed"));
    ...
    assert.match(softwareHead, /name="keywords" content="GodLock Softwares, GodLock, GodLock.uk/);
    assert.match(softwareHead, /og:title" content="GodLock Softwares"/);
    assert.doesNotMatch(softwareHead, /Trades-Runtime/);
...
describe("Aziel Corpus Library off-site", () => {
  it("points the nav tab at the live library identity URL", () => {
    ...
    assert.match(nav, /href="https:\/\/www\.hedidntjump\.com\/" class="aziel">He Didn't Jump<\/a>/);
  ...
  it("serves that absolute library href from the homepage", async () => {
    ...
    assert.match(html, /href="https:\/\/www\.hedidntjump\.com\/" class="aziel">He Didn't Jump<\/a>/);
...
function suiteCardsHtml(products, extras) {
  return softwareSuite(products, extras).map((p) => softwaresCard(p)).join("");
}
...
describe("Software page hosts the full aziel-runtime catalog", () => {
  ...
  it("inherits SpectralLock leftover-bytes + recover + handwriting honesty after spectrallock#13", () => {
    ...
    assert.match(llms, /pixel heuristics/);
  ...
  it("auto-includes live catalog slugs the snapshot does not know yet", async () => {
    ...
    const publicHtml = softwareBody({ products: fetched.products });
    assert.deepEqual(softCardIds(publicHtml), ["godlock", "aziel-runtime"]);
    assert.match(publicHtml, /https:\/\/www\.azieleliab\.com\/software/);
    const html = suiteCardsHtml(fetched.products);
  ...
  it("renders catalog cards with Worker, GitHub, Runtime, MCP, and counters when present", () => {
    const runtimeExtras = { runtimeUses: 42 };
    const products = CATALOG_FALLBACK_PRODUCTS.map((p) => (
      p.slug === "godlock" ? { ...p, downloads: 40, uses: 7 } : p
    ));
    const pageHtml = softwareBody({ products, extras: runtimeExtras });
    assert.deepEqual(softCardIds(pageHtml), ["godlock", "aziel-runtime"]);
    assert.match(pageHtml, /id="godlock"/);
    assert.doesNotMatch(pageHtml, /id="fraggate"/);
    assert.match(pageHtml, /<h2 class="soft-heading">Softwares<\/h2>\s*<div class="soft-grid">/);
    const html = suiteCardsHtml(products, runtimeExtras);
    const ids = softCardIds(html);
    ...
    assert.match(html, /href="\/">Open Engine<\/a>/);
    assert.doesNotMatch(html, /href="\/runtime\/v1\/pull\/godlock">Invoke via Runtime<\/a>/);
    ...
    assert.match(pageHtml, /href="\/">Open Engine<\/a>/);
  ...
  it("serves GodLock-first Softwares HTML from GET /software even when origin fetch is empty", async () => {
    const res = await worker.fetch(new Request("https://godlock.uk/software"), mockEnv());
    assert.equal(res.status, 200);
    const html = await res.text();
    const ids = softCardIds(html);
    const names = softCardH3s(html);
    assert.deepEqual(ids, ["godlock", "aziel-runtime"]);
    assert.deepEqual(names, ["GodLock", "Aziel Runtime"]);
    assert.match(html, /<article class="soft-card featured" id="godlock"/);
    assert.doesNotMatch(html, /<title>[^<]*Trades-Runtime/);
    assert.doesNotMatch(html, /Aziel Runtime only on this page/);
    assert.doesNotMatch(html, /id="fraggate"/);
    assert.doesNotMatch(html, /id="azbrowser"/);
    assert.match(html, /<title>GodLock Softwares<\/title>/);
    assert.match(html, /href="\/runtime">Runtime<\/a>/);
    assert.match(html, /class="button" href="https:\/\/glama\.ai\/mcp\/servers\/AzielEliab\/aziel-runtime">Try on Glama<\/a>/);
    assert.match(html, /class="button ghost" href="\/runtime">Official Runtime<\/a>/);
    ...
    assert.deepEqual(cite.software_slugs, ["aziel-runtime"]);
    assert.equal(cite.software_product_count, 1);
    ...
    assert.match(defaultDescription("software"), /first card is GodLock/);
    assert.doesNotMatch(defaultDescription("software"), /Aziel Runtime only on this page/);
    assert.doesNotMatch(defaultDescription("software"), /Trades-Runtime/);
    assert.match(defaultDescription("software"), /https:\/\/www\.azieleliab\.com\/software/);
  ...
  it("renders Softwares HTML from the packed catalog without waiting on a slow upstream", async () => {
    ...
    assert.match(html, /<h2 class="soft-heading">Softwares<\/h2>\s*<div class="soft-grid">/);
    assert.match(html, /<title>GodLock Softwares<\/title>/);
    assert.match(html, /name="description" content="GodLock Softwares is GodLock-first/);
    assert.doesNotMatch(html, /Aziel Runtime only on this page/);
    assert.doesNotMatch(html, /<title>[^<]*Trades-Runtime/);
    assert.match(html, /https:\/\/www\.azieleliab\.com\/software/);
    assert.doesNotMatch(html, /Suite doors available from GodLock/);
    ...
    assert.doesNotMatch(html, /id="azcoherence"/);
    assert.doesNotMatch(html, /id="trades-runtime"/);
  ...
  it("serves GET /v1/software with AZCoherence in Plain A–Z even from the local fallback", async () => {
    ...
    assert.ok(api.products.every((p) => p.slug === "aziel-runtime"));
  ...
  it("lists AZBrowser, AZNet, and FragGate as separate cards with Worker /count pills", async () => {
    ...
    const page = softwareBody({ products: counted.products });
    assert.deepEqual(softCardIds(page), ["godlock", "aziel-runtime"]);
    assert.doesNotMatch(page, /id="fraggate"/);
    const html = suiteCardsHtml(counted.products);
...
describe("Phase F Aziel Runtime 2.0.0-rc1 hub cite", () => {
  it("leads Softwares and runtime SEO with the crawler abstract and cites live 2.0.0-rc1", () => {
    ...
    assert.match(RUNTIME_ABSTRACT, /node-meshed orchestration suite/);
    const software = defaultDescription("software");
    const runtime = defaultDescription("runtime");
    assert.match(software, /first card is GodLock/);
    assert.doesNotMatch(software, /Aziel Runtime is not merely an API orchestrator/);
    assert.doesNotMatch(software, /Aziel Runtime only on this page/);
    assert.doesNotMatch(software, /Trades-Runtime/);
    assert.ok(runtime.startsWith(RUNTIME_ABSTRACT.slice(0, 80)));
    assert.match(software, /https:\/\/www\.azieleliab\.com\/software/);
    assert.doesNotMatch(software, /Suite doors available from GodLock/);
...
describe("Aziel Public Entity Graph Phases B–D", () => {
  ...
  it("puts the ecosystem block in footer/nav, not between Softwares heading and list", async () => {
    ...
    assert.match(eco, /href="https:\/\/www\.hedidntjump\.com\/">He Didn't Jump<\/a>/);
    ...
    const runtimeItem = (list.itemListElement || []).find((it) => it.item && it.item.identifier === "aziel-runtime");
    assert.ok(runtimeItem);
    assert.ok(isSharedPersonRef(runtimeItem.item.author));
    const siteGod = liveLd["@graph"].find((n) => n["@type"] === "SoftwareApplication" && n["@id"] === "https://godlock.uk/#godlock");
    assert.ok(siteGod);
    const toolParts = [].concat(siteGod.isPartOf || []);
    assert.ok(toolParts.some((p) => p && p["@id"] === "https://www.azieleliab.com/runtime#godlock"));
    assert.ok(isSharedPersonRef(siteGod.author));
  });
...
describe("GodLock Softwares identity", () => {
  it("keeps GodLock HTTPS-engine chrome and features GodLock first without library-hub copy", () => {
    assert.deepEqual(FEATURED_SOFTWARES, ["godlock"]);
    const suite = softwareSuite(CATALOG_FALLBACK_PRODUCTS);
    assert.equal(suite[0].slug, "aziel-runtime", "catalog JSON stays Plain→Gate→Lock extras-first");
    const pageCards = featureGodLockFirst(publicSoftwaresHtmlList(suite));
    assert.deepEqual(pageCards.map((p) => p.slug), ["godlock", "aziel-runtime"]);
    ...
    const cite = citeDoc();
    assert.match(cite.software_suite, /first card is GodLock/);
    assert.match(cite.software_suite, /Aziel Runtime only plus sister_cites/);
    assert.match(cite.software_suite, /https:\/\/www\.azieleliab\.com\/software/);
    assert.doesNotMatch(cite.software_suite, /Suite doors available from GodLock/);
...
describe("receipts UI and public payload", () => {
  it("lists prior public receipts in caller order (newest-first from publicReceipts)", () => {
    ...
    assert.match(home, /id="Nodes">0</);
    assert.match(home, /id="LiveNodes">0</);
    assert.match(home, /Nodes \/ Live Nodes/);
