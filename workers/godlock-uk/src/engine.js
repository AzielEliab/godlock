...
export function fallbackAnswer(text, currentScore, priorNodes) {
  ...
  if (!t) {
    ...
    explanation = "GodLock.uk is a public HTTPS bootstrap engine. Suite mesh is QNM-BUILD-1.0: read-only suite presence on, live|locked|isolated counts only. No Node Gate. No auto-heal. SPLIT THE WIRES: tip-only 0.5–1s tick; pull-only payload; Phoenix is local only and die-with-pull does not bring godlock.uk back. COLD-COPY SURVIVAL: cold copies multiply; live sync is refused; a server pull does not erase records. REHEAL refuse: a poisoned node uses its own last good tip plus a verified trusted pull, or phoenix-WAIT — no neighbor talk-back-to-health; bodies, diffs, and vote-to-fix are forbidden. Softwares stays Runtime-only. It is not an anonymity network, VPN, Tor hop, or tunnel. Treating this HTTPS page as a live hop mesh is a category error. The engine records the claim and lowers confidence slightly because the public copy must stay honest." + cite;
